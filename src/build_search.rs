...
pub fn run(html_dir: &str) -> anyhow::Result<()> {
    ...
    // 1. 扫描 HTML 文件
    log::debug!("扫描 HTML 文件...");
    let html_files = scan_html_files(html_path, "");
    log::debug!("  找到 {} 个 HTML 文件", html_files.len());
    ...
    log::debug!("  提取 {} 个文档", docs.len());

    // 3. 构建倒排索引（bigram 分词）
    log::debug!("构建搜索索引（中文 bigram 分词）...");
    ...
    log::debug!("  输出 searchindex.js ({:.0} KB)", js_content.len() as f64 / 1024.0);

    // 5. 输出 searcher.js
    let searcher_path = html_path.join("searcher.js");
    fs::write(&searcher_path, SEARCHER_JS)?;
    log::debug!("  输出 searcher.js（中文 bigram 搜索版）");
    ...
    log::debug!("  已清理 {} 个 HTML 文件中的旧搜索库引用", replaced);

    log::debug!("中文搜索索引构建完成！");
    Ok(())
}
