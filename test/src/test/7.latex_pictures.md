...
使用 [rust-tikzjax](https://github.com/siefkenj/rust-tikzjax) 在构建时编译 TikZ 图片为内联 SVG（基于 WebAssembly 的 TeX 引擎）。
...
- chemfig（化学结构式）
- tikz-3dplot（3D 绘图）
- 标准 TikZ 库（decorations, arrows, shapes 等）
...
\newtemplate\degensink{\shortstack{degenerate\\ sink}}%
 {\draw (0,-4) -- (0,4);                  % draw vertical axis
...
\newtemplate\degensource{\shortstack{degenerate\\ source}}%
 {\draw (0,-4) -- (0,4);                  % draw vertical axis
  \foreach \s in {+,-}                    % for upper/lower half do:
...
\begin{tikzpicture}[line cap=round,line join=round]
  ...
  \draw [main,->] (0,-0.3) -- (0,4.7)                            % vertical axis
    ...
    node [label={[above,yshift=0.8cm,align=left]%
      {\sffamily\large Poincar\'e Diagram: Classification of Phase\\ Portraits in the $(\det A,\Tr A)$-plane}}] {};
