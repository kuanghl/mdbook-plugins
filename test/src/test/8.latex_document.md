...
\usepackage{amsmath,textgreek,textcomp,gensymb,amssymb}

% ==== 兼容层：latex.js 特有/缺失命令在标准 LaTeX (tectonic) 下的兜底 ====
% \symbol{"00A9} 是 latex.js 扩展；\providecommand 保证 latex.js 下不覆盖其
% 已有定义，tectonic 下则退化为 TeX 原语 \char（" 开头即十六进制）。
\providecommand{\symbol}[1]{\char#1}
% \varheartsuit 原由 stix 提供（本文档已改用 amssymb 以避开 stix/textcomp
% 选项冲突）；tectonic 下退化为内核 \heartsuit，latex.js(KaTeX) 下保持原有
% 空心心形符号。
\providecommand{\varheartsuit}{\heartsuit}
...
\begin{itemize}
    ...
    \item \verb|^^A9 or ^^^^00A9|: \char"A9\ or \char"00A9
\end{itemize}
...
\noindent\fbox{\texttt{\textbackslash mbox\{}\emph{text}\texttt{\}}}
...
\noindent\fbox{\texttt{\textbackslash makebox[}\emph{width}\texttt{][}\emph{pos}\texttt{]\{}\emph{text}\texttt{\}}}

\noindent\fbox{\texttt{\textbackslash framebox[}\emph{width}\texttt{][}\emph{pos}\texttt{]\{}\emph{text}\texttt{\}}}
...
\noindent\fbox{\texttt{\textbackslash parbox[}\emph{pos}\texttt{][}\emph{height}\texttt{][}\emph{inner-pos}\texttt{]\{}\emph{width}\texttt{\}\{}\emph{text}\texttt{\}}}
...
\noindent
\llap{\texttt{\textbackslash llap }}could be used to put something in the margin. However, there are better alternatives for that.
